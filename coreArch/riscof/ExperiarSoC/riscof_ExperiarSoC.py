...
class ExperiarSoC(pluginTemplate):
	__model__ = "ExperiarSoC"
	...
	def __init__(self, *args, **kwargs):
		...
		self.dut_exe = f"python3 {os.path.join(config['PATH'] if 'PATH' in config else '', 'runArchTest.py')}"
	...
	def build(self, isa_yaml, platform_yaml):
		...
		# for ExperiarSoC start building the '--isa' argument. the self.isa is dutnmae specific and may not be
		# useful for all DUTs
	...
	def runTests(self, testList):
		...
		# function earlier
		make.makeCommand = 'make -j' + self.num_jobs + " GCC_PATH=/opt/riscv32i/bin GCC_PREFIX=riscv32-unknown-elf"
		...
		for testname in testList:
			...
			elf = os.path.join(test_dir, "coreArch.elf")
			...
			# echo statement.
			if self.target_run:
				simcmd = self.dut_exe + ' isa={0} signature={1} elf={2} > log.txt'.format(self.isa, sig_file, elf)
			else:
				simcmd = 'echo "NO RUN"'
